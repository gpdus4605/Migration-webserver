...
# 'api' 서비스와 동일하게 .env 파일의 변수들을 조합하고, flask 명령어의 절대 경로를 사용하여 실행합니다.
docker-compose -f /home/ubuntu/onpremise-webservice/docker-compose.yml -p onpremise exec -T api sh -c "export DATABASE_URL=postgresql://$POSTGRES_USER:$POSTGRES_PASSWORD@db:5432/$POSTGRES_DB && /usr/local/bin/flask db upgrade"
